...
const useIPadCursorInit = (config) => {
    useLayoutEffect(() => {
        initCursor(config);
        return () => {
            disposeCursor();
...
function IPadCursorProvider({ children, config, }) {
    useIPadCursorInit(config);
    return (React.createElement(CursorContext.Provider, { value: useRef({
            CursorType,
            resetCursor,
            disposeCursor,
            initCursor,
...
function useIPadCursor() {
    return React.useContext(CursorContext);
}

export { IPadCursorProvider, useIPadCursor };
